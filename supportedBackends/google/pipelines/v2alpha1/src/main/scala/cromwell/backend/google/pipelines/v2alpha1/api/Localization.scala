...
trait Localization {
  def localizeActions(createPipelineParameters: CreatePipelineParameters, mounts: List[Mount])(implicit localizationConfiguration: LocalizationConfiguration) = {
    val groupLocalizations = true

    val jobInputLocalization =
      if (groupLocalizations) PipelinesParameterConversions.groupedLocalizationActions(createPipelineParameters.inputOutputParameters.fileInputParameters, mounts)
      else createPipelineParameters.inputOutputParameters.fileInputParameters.flatMap(_.toActions(mounts).toList)
