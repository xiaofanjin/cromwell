...
import cromwell.services.metadata.MetadataService.{MetadataWriteAction, MetadataWriteFailure, WorkflowMetadataReadAction}
...
class CarboniteMetadataServiceActor(carboniteConfig: HybridCarboniteConfig, serviceRegistryActor: ActorRef) extends Actor with ActorLogging with GracefulShutdownHelper {
  ...
  override def receive: Receive = {
    case read: WorkflowMetadataReadAction =>
      ioActorOption match {
        case Some(ioActor) =>
